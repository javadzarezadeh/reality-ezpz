    ...
    cat >"${path[engine]}" <<EOF
{
  ...
  "inbounds": [
    ...
    },
    {
      ...
      echo '"strict_mode": false, "detour": "shadowsocks", "handshake": {"server": "'"${config[domain]%%:*}"'", "server_port": '"${reality_port}"'}'
      fi
      )
    }
    $(if [[ ${config[transport]} == 'shadowtls' ]]; then
